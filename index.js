...
let DB_URI;
if (process.env.NODE_ENV === process.env.DB_URI_TEST) {
  DB_URI = process.env.DATABASE_SECRET;
} else {
  ...
  if (req.isAuthenticated()) {
    try {
      ...
      if (secret) {
        ...
        res.render("secrets.ejs", { secret: "You have not submitted a secret yet." });
      }
    } catch (error) {
...
// Ropute to handle secret submission
...
passport.use("google", new GoogleStrategy({
  ...
  try {
    ...
    if (result.rows.length > 0) {
      return cb(null, result.rows[0]); // User already exists, return the user
    } else {
      const newUser = await db.query("INSERT INTO users (email) VALUES ($1, $2) RETURNING *", [profile.emails[0].value, profile.id || "google"]);
